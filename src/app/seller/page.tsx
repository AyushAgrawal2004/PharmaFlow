...
export default function SellerNewOrderPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* Header */}
      <div className="flex items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200/60 shadow-xs">
        <div className="p-3.5 bg-teal-50 text-teal-600 rounded-2xl border border-teal-100/60">
          <Stethoscope className="w-6 h-6 animate-pulse" />
        </div>
        <div>
          <h1 className="text-xl font-black tracking-tight text-slate-800">Dispensing Panel</h1>
          <p className="text-xs font-medium text-slate-400 mt-0.5">Search pharmacy catalog, configure dosage metrics, and generate digital quotations</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-xs space-y-3.5">
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Prescription Search</span>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/25 focus:border-teal-500 transition-all text-xs font-semibold text-slate-700 placeholder-slate-400 input-focus-ring"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {loadingCatalogue ? (
              ...
              <div className="col-span-2 text-center py-20 text-slate-400 bg-white border border-slate-200/60 rounded-3xl p-5 shadow-xs">
              ...
              products.map((product) => {
                ...
                return (
                  <div key={product.id} className="bg-white p-5 rounded-3xl border border-slate-200/60 shadow-xs flex flex-col justify-between gap-5 transition-all hover:-translate-y-0.5 hover:shadow-md hover:border-slate-250 group">
                    <div>
                      <div className="flex items-start justify-between gap-2.5">
                        <h3 className="font-extrabold text-slate-800 text-sm group-hover:text-teal-700 transition-colors line-clamp-2">{product.name}</h3>
                        <span className="text-[9px] font-mono bg-slate-50 border border-slate-200 text-slate-500 px-1.5 py-0.5 rounded font-bold uppercase shrink-0">
                          {product.sku}
                        </span>
                      </div>
                      <span className="text-[10px] font-bold text-slate-400 block mt-1 uppercase tracking-wider">{product.category || 'General'}</span>
                      
                      {minStr && (
                        <div className="mt-3.5">
                          <span className="inline-flex items-center gap-1 text-[9px] font-extrabold text-indigo-650 bg-indigo-50/70 border border-indigo-100 px-2 py-0.5 rounded-lg select-none">
                            Min Purchase: {minStr}
                          </span>
                        </div>
                      )}

                      <div className="mt-4 flex items-baseline justify-between gap-2 border-t border-slate-105 pt-3">
                        <div>
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Unit Price</span>
                          <span className="text-xs font-black text-slate-700">{priceStr}</span>
                        </div>
                        <div className="text-right">
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">In Stock</span>
                          <span className={`text-xs font-bold ${outOfStock ? 'text-rose-600' : 'text-slate-500'}`}>{stockStr}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      disabled={outOfStock}
                      className="w-full py-2.5 rounded-xl bg-teal-50 hover:bg-teal-100 text-teal-700 disabled:opacity-40 disabled:pointer-events-none active:scale-[0.98] font-bold text-[10px] tracking-wider uppercase border border-teal-150 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                    >
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden sticky top-[80px]">
          ...
          <div className="p-5 flex flex-col gap-4 max-h-[58vh] overflow-y-auto min-h-[30vh]">
            {cart.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center text-slate-400 py-16 gap-3">
                ...
                <div>
                  <h4 className="font-bold text-slate-700 text-xs">Dispense Draft is Empty</h4>
                  <p className="text-[10px] text-slate-400 mt-1 max-w-[220px] mx-auto font-medium">Select medications from the chemical catalog on the left to start compounding your order draft.</p>
                </div>
              </div>
            ) : (
              cart.map((item) => {
                ...
                return (
                  <div key={item.product.id} className={`p-4 rounded-2xl border transition-colors space-y-3.5 relative overflow-hidden group ${
                    isViolation ? 'bg-rose-50/40 border-rose-200' : 'bg-slate-50 border-slate-200/50'
                  }`}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h4 className="font-black text-slate-800 text-xs leading-snug">{item.product.name}</h4>
                        <div className="flex items-center gap-2.5 mt-1">
                          <span className="text-[9px] text-slate-400 font-mono font-bold">SKU: {item.product.sku}</span>
                          <span className="text-[9px] text-slate-300">•</span>
                          <span className="text-[10px] font-bold text-slate-500 uppercase">{item.product.category || 'General'}</span>
                        </div>
                      </div>
                      <button
                        onClick={() => removeFromCart(item.product.id)}
                        className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg border border-transparent hover:border-rose-100 transition-all cursor-pointer shrink-0"
                        title="Remove"
                      >
                    ...
                    <div className="flex items-center justify-between bg-white border border-slate-150 px-2.5 py-1.5 rounded-lg">
                      <div className="flex items-center gap-1.5">
                        <Info className="w-3.5 h-3.5 text-teal-605 shrink-0 text-teal-650" />
                      ...
                      {displayMinQty > 0 && (
                        <span className="text-[9px] font-bold text-indigo-650 bg-indigo-50 px-1.5 py-0.5 rounded">
                          Min: {displayMinQty} {item.unit}
                        </span>
                      )}
                    </div>

                    <div className="flex flex-row items-center justify-between gap-3 pt-2.5 border-t border-slate-200/60">
                      <div className="flex flex-col items-start gap-1">
                        <div className="flex items-center rounded-lg border border-slate-250 bg-white overflow-hidden shadow-xs h-8">
                          <button
                            onClick={() => updateQuantity(item.product.id, Math.max(0.000001, item.quantity - 0.1))}
                            className="px-2 hover:bg-slate-50 transition-colors text-slate-500 h-full border-r border-slate-200 cursor-pointer"
                          >
                          ...
                          <input
                            ...
                            className="w-16 text-center text-[10px] font-black text-slate-800 focus:outline-none h-full"
                          />
                          <button
                            onClick={() => updateQuantity(item.product.id, item.quantity + 0.1)}
                            className="px-2 hover:bg-slate-50 transition-colors text-slate-500 h-full border-l border-slate-200 cursor-pointer"
                          >
                        ...
                        <select
                          ...
                          className="mt-1 h-8 py-1 pl-2 pr-6 rounded-lg border border-slate-250 text-[10px] focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all bg-white font-bold text-slate-650 cursor-pointer"
                        >
                          {unitOptions.map(opt => (
                      ...
                      <div className="text-right">
                        <span className="text-[9px] font-bold text-slate-400 block uppercase tracking-widest">Subtotal</span>
                        <span className="text-xs font-extrabold text-slate-800">{formatPrice(subtotal)}</span>
                      </div>
                    </div>

                    {isViolation && (
                      <div className="text-[10px] text-rose-600 font-extrabold bg-rose-50 border border-rose-100 p-2 rounded-xl text-center">
          ...
          {cart.length > 0 && (
            <div className="p-5 bg-slate-50 border-t border-slate-200/80 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200/60 pb-3">
                <span className="font-bold text-slate-500 text-xs tracking-wider uppercase">Prescription Total</span>
                <span className="text-lg font-black text-slate-800 tracking-tight">{formatPrice(cartTotal)}</span>
              </div>

              {hasMinPurchaseViolations && (
                <div className="p-3 bg-rose-50 border border-rose-100 rounded-xl text-rose-600 font-extrabold text-[10px] leading-relaxed text-center">
              ...
              <button
                ...
                className="w-full py-3.5 px-4 rounded-xl bg-slate-950 hover:bg-slate-850 text-white font-bold text-xs tracking-wider uppercase shadow-md transition-all active:scale-[0.98] flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                {checkoutLoading ? (
                  <>
